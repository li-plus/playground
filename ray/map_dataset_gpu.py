...
class TorchPredictor:
    ...
    def __call__(self, batch: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        batch_messages = [
            [{"role": "user", "content": f"Count from {s} to {e}: "}] for s, e in zip(batch["start"], batch["end"])
        ]
        inputs = self.tokenizer.apply_chat_template(
            batch_messages, add_generation_prompt=True, tokenize=True, return_tensors="pt", return_dict=True
        ).to("cuda")
